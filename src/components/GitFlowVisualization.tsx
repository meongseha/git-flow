...
export const GitFlowVisualization: React.FC<GitFlowVisualizationProps> = ({
  ...
  const infoColor = isDark ? "#9ca3af" : "#666";
  ...
  useEffect(() => {
    ...
    if (currentStep >= 3) {
      ...
      if (currentStep >= 4) {
        ...
        newBranches = newBranches.map((branch) =>
          branch.name === "release/1.0" ? { ...branch, endPosition: 6 } : branch
        );
      }
    }
    ...
    if (currentStep >= 5) {
      ...
      if (currentStep >= 6) {
        ...
        newBranches = newBranches.map((branch) => {
          if (branch.name === "feature/profile")
            return { ...branch, endPosition: 14 };
          if (branch.name === "feature/dashboard")
            return { ...branch, endPosition: 15 };
          return branch;
        });
  ...
  const renderBranchLines = () => {
    return branches.map((branch, index) => {
      const startCommit = commits.find(
        (c) => c.position === branch.startPosition
      );
      ...
      const startPos = getCommitPosition(
        startCommit,
        startBranch >= 0 ? startBranch : 0
      );
      const endPos = endCommit
        ? getCommitPosition(endCommit, index)
        : {
            x: 50 + index * 80,
            y: 40 + Math.max(...commits.map((c) => c.position)) * 60 + 60,
          };
      ...
      return (
        <g key={commit.id} className="commit-node">
          ...
          <text
            ...
            y={pos.y - 10}
            textAnchor="start"
            fill={branches[branchIndex].color}
            fontWeight="bold"
            fontSize="15"
            style={{ filter: isDark ? "drop-shadow(0 1px 2px #0008)" : "none" }}
          >
  ...
  return (
    <div className="w-full overflow-auto" style={{ maxHeight: "500px" }}>
      <svg width={svgWidth} height={svgHeight} style={{ minWidth: "100%" }}>
        {renderBranchLines()}
        {renderCommits(labelColor, infoColor, isDark)}
      </svg>
    </div>
  );
};
