    ...
    const interval = setInterval(() => {
      ...
      if (time % 10 === 0) {
        ...
        setDevelopers(prevDevelopers => {
          ...
          if (developer.currentBranch !== 'develop' && 
          ...
          newDevelopers.forEach(dev => {
            if (dev.id === 3 && Math.random() > 0.7) { // Charlie reviews PRs
              ...
              if (openPRs.length > 0) {
                const prToReview = openPRs[Math.floor(Math.random() * openPRs.length)];
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      <div className="col-span-1 md:col-span-3 flex items-center justify-between mb-2">
        <div className="flex items-center space-x-2">
          ...
          <button
            ...
            className={`px-2 py-1 text-xs rounded ${speed === 0.5 ? 'bg-blue-100 text-blue-700' : 'bg-gray-100'}`}
          >
            Slow
          </button>
          <button
            onClick={() => setSpeed(1)}
            className={`px-2 py-1 text-xs rounded ${speed === 1 ? 'bg-blue-100 text-blue-700' : 'bg-gray-100'}`}
          >
            Normal
          </button>
          <button
            onClick={() => setSpeed(2)}
            className={`px-2 py-1 text-xs rounded ${speed === 2 ? 'bg-blue-100 text-blue-700' : 'bg-gray-100'}`}
          >
            Fast
          </button>
        </div>
        <button
          ...
          className={`px-3 py-1 text-xs rounded ${isPaused ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}
        >
      ...
      {developers.map(developer => (
        <div key={developer.id} className="bg-gray-50 rounded-lg p-4">
          ...
          <div className="mb-4">
            ...
            {developer.commits.length === 0 ? (
              <p className="text-sm text-gray-500 italic">No recent commits</p>
            ) : (
              <ul className="text-sm space-y-2">
                {developer.commits.map(commit => (
                  <li key={commit.id} className="bg-white p-2 rounded border border-gray-200">
                    <div className="flex justify-between">
                      <span className="font-mono text-xs text-gray-600">{commit.id}</span>
          ...
          <div>
            ...
            {developer.pullRequests.length === 0 ? (
              <p className="text-sm text-gray-500 italic">No pull requests</p>
            ) : (
              <ul className="text-sm space-y-2">
                {developer.pullRequests.map(pr => (
                  <li key={pr.id} className="bg-white p-2 rounded border border-gray-200">
                    <div className="flex justify-between">
